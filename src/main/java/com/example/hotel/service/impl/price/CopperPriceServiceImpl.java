...
@AllArgsConstructor
@Slf4j
public class CopperPriceServiceImpl implements PriceCalculationService {

  @Override
  public PriceResponse calculateOrderPrice(PrebookRoomRequestDTO requestDTO,
      BigDecimal roomTypePrice, String userId) throws BizException {
    ...
    BigDecimal realRoomPrice = roomTypePrice.multiply(CommonConstant.COPPER_DISCOUNT).setScale(2, RoundingMode.HALF_UP);
    BigDecimal memberPrice = totalPrice.multiply(CommonConstant.COPPER_DISCOUNT).setScale(2, RoundingMode.HALF_UP);
...
//    BigDecimal realPrice = memberPrice.subtract(pointsDiscount).setScale(2, RoundingMode.HALF_UP);
    ...
    Integer earnPointCount = memberPrice.multiply(CommonConstant.COPPER_POINTS_RULE).setScale(0, RoundingMode.HALF_UP).intValue();
