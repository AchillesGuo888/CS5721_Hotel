...
public class GoldPriceServiceImpl implements PriceCalculationService {
  ...
  public PriceResponse calculateOrderPrice(QueryOrderAmountRequestDTO requestDTO)
    ...
    BigDecimal realRoomPrice = requestDTO.getRoomTypePrice().multiply(dates)
        .multiply(CommonConstant.GOLD_DISCOUNT).setScale(2, RoundingMode.HALF_UP);
    BigDecimal memberPrice = totalPrice.multiply(CommonConstant.GOLD_DISCOUNT)
        .setScale(2, RoundingMode.HALF_UP);
...
//    BigDecimal realPrice = memberPrice.subtract(pointsDiscount).setScale(2, RoundingMode.HALF_UP);
    BigDecimal membershipDiscount = totalPrice.subtract(memberPrice)
        .setScale(2, RoundingMode.HALF_UP);
    Integer earnPointCount = memberPrice.multiply(CommonConstant.GOLD_POINTS_RULE)
  ...
  public Integer getEarnPoints(BigDecimal orderPrice) {
    ...
    Integer earnPointCount = orderPrice.multiply(CommonConstant.GOLD_POINTS_RULE)
        .setScale(0, RoundingMode.HALF_UP).intValue();
