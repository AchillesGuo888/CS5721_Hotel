...
public class GoldPriceServiceImpl implements PriceCalculationService {
  ...
  public PriceResponse calculateOrderPrice(PrebookRoomRequestDTO requestDTO,
    ...
    BigDecimal realRoomPrice = roomTypePrice.multiply(CommonConstant.GOLD_DISCOUNT).setScale(2, RoundingMode.HALF_UP);
    BigDecimal memberPrice = totalPrice.multiply(CommonConstant.GOLD_DISCOUNT).setScale(2, RoundingMode.HALF_UP);
...
//    BigDecimal realPrice = memberPrice.subtract(pointsDiscount).setScale(2, RoundingMode.HALF_UP);
    ...
    Integer earnPointCount = memberPrice.multiply(CommonConstant.GOLD_POINTS_RULE).setScale(0, RoundingMode.HALF_UP).intValue();
