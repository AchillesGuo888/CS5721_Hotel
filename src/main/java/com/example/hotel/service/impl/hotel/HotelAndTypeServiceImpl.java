...
public class HotelAndTypeServiceImpl implements HotelAndTypeService {
  ...
  public List<AvailableHotelResponse> queryHotelListWithPrice(QueryHotelRequestDTO requestDTO) {
    ...
    List<HotelInfo> hotelInfoList = getHotelList(requestDTO.getName(), requestDTO.getAddress(),
        requestDTO.getCity());
    ...
    Map<Long, BigDecimal> typeAndPriceMap = roomTypePriceService
        .getHotelsRoomTypeAndPrice(roomTypeIds, requestDTO.getStartDate(),
  ...
  public List<RoomAndTypeWithPriceResponse> getHotelAvailableRoomWithPrice(
    ...
    //Map<roomTypeId,price>
    Map<Long, BigDecimal> typeAndPriceMap = roomTypePriceService
        .getHotelsRoomTypeAndPrice(roomTypeIds
            , requestDTO.getStartDate(),
            requestDTO.getEndDate());

    //assemble data
    List<RoomAndTypeWithPriceResponse> response = assembleRoomTypePrice(roomTypeInfoList,
        typeAndPriceMap);
  ...
  //
  private List<RoomAndTypeWithPriceResponse> assembleRoomTypePrice(
      List<AvailableRoomCountDTO> roomTypeInfoList, Map<Long, BigDecimal> typeAndPriceMap) {
  ...
  private List<AvailableHotelResponse> assembleHotelPrice(
    ...
    for (AvailableRoomCountDTO item : hotelRoomTypeList) {
      ...
      if (hotelRoomTypePriceMap.get(item.getHotelId()).compareTo(BigDecimal.ZERO) == 0) {
        ...
        if (hotelRoomTypePriceMap.get(item.getHotelId())
            .compareTo(typeAndPriceMap.get(item.getRoomTypeId())) > 0) {
          hotelRoomTypePriceMap.put(item.getHotelId(), typeAndPriceMap.get(item.getRoomTypeId()));
        }
      }
    }
    ...
    hotelMap.forEach((key, value) -> {
      ...
      AvailableHotelResponse obj = AvailableHotelResponse.builder()
          .hotelId(value.getId()).hotelName(value.getHotelName()).price(hotelPrice)
          .status(hotelStatus)
          .build();
      result.add(obj);
    });
